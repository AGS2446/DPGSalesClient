...
$('#EnquiryMaturityDate').datepicker({
    ...
    //if ($('#EnquiryValidityDate').val() == "") {
    //    AlertPopup('Please select Enquiry Valid Date first');
    //    $('#EnquiryMaturityDate').val('');

    //} else if (ConvertStrDate($('#EnquiryMaturityDate').val()) > ConvertStrDate($('#EnquiryValidityDate').val())) {
    //    AlertPopup('Maturity date should be less than or equal to Valid date (' + $('#EnquiryValidityDate').val() + ')');
    //    $('#EnquiryMaturityDate').val('');
    //}
});

//$('#EnquiryValidityDate').datepicker({
//    autoclose: true,
//    format: 'd/m/yyyy',
//    startDate: "today"
//});
...
function FillPlants(strDivId,strBrchId) {
    ...
    $.post('AssignedToUsers', { strDivision: strDivId, strBranch: strBrchId }).done(function (data) {
        ...
        if (data != null && data != undefined) {
            ...
            $('#AssignedToUser').empty();
            if (data.length > 1)
                $('#AssignedToUser').append('<option value="">SELECT</option>');
            debugger;
            for (var i = 0; i < data.length; i++) {
                $('#AssignedToUser').append('<option value="' + data[i].text + '#' + data[i].value + '">' + data[i].text + '</option>')
            }
        }
    }).always(function () {
        $("#initialload").hide();
...
$('#btnAddProduct').click(function () {
    debugger;
        ...
        //create
        var validator = $("#enqCreateForm").validate();
        var isValid = validator.form();
        var data = $("#enqCreateForm").serialize();

        if (isValid) {
            $.post('NavigateEnquiryItems', data).done(function (data) {
                if (data != null) {
                    if (data.status == "FAILED") {
                        AlertPopup('Please provide all reuired data');
                    } else {
                        debugger;
                        window.location = "EnquiryItems?BusinessSegment=" + data.businessSegment;
                    }
                }
...
$('#btnAddProductEdit').click(function () {
    ...
    //create
    var validator = $("#enqEditForm").validate();
    var isValid = validator.form();
    var data = $("#enqEditForm").serialize();
        ...
        $.post('NavigateEnquiryItems', data).done(function (data) {
            if (data != null) {
                if (data.status == "FAILED") {
                    ...
                    debugger;
                    window.location = "EnquiryItems?BusinessSegment=" + data.businessSegment + "&enqId=" + data.enquiryId;
                }
            }
...
$('#btnContractValueSave').click(function () {
    ...
    if ($('#txtContractValue').val() != null && $('#txtContractValue').val() != "")
        if (isValidContractValueAddress($('#txtContractValue').val())) {
            $.post('../EnquiryOpportunity/UpdateContractValue', { enqId: $('#EnquiryID').val(), contractValue: parseFloat($('#txtContractValue').val()) }).done(function (data) {
                if (data != null) {
                    if (data.status == "SUCCESS") {
                        $('#contractModal').modal('hide');
                        
                        //window.location = "Details?enqId=" + $('#EnquiryID').val() + "&status=" + $('#Status').val();
                        window.location = "Edit?enqId=" + $('#EnquiryID').val() + "&stBack=";
...
jQuery.validator.addMethod("DigitsTwoDecimal", function (value, element) {
    ...
    $("#enqCreateForm").validate({
        ...
        messages: {
            ...
            EnquiryValidityDate: {
                required: "Please provide Enquiry Validity Date"
            },
            EnquiryMaturityDate: {
                required: "Please provide Enquiry Maturity Date"
            },
            AssignedToUser: {
