...
/* ── Dropdown: show mandatory hint ────────────────────────── */
$('#ddlDocumentType').on('change', function () {
    debugger
    var selected = $(this).find('option:selected');
    var isMandatory = selected.data('mandatory') == 1;
    ...
    /* clear invalid styling when user picks a value */
    if ($(this).val() !== '') {
        $(this).removeClass('is-invalid');
    }

    /* sync hidden field so model binding picks it up on submit */
    $('#hidDocumentType').val($(this).val());
});


/* ── Add button (unchanged wiring) ────────────────────────── */
...
function handleFileSelect() {
    ...
    /* ── NEW: validate dropdown first ── */
    var docType = $('#ddlDocumentType').val();
    var isDirect = $('#isDirect').val();
    if ((!docType || docType === '') && isDirect.toLowerCase()!="true") {
        $('#ddlDocumentType').addClass('is-invalid');
        AlertPopup('Please select a Document Type before adding a file.');
        return;
    }
    $('#ddlDocumentType').removeClass('is-invalid');
    /* ── end dropdown check ── */
        ...
        for (var intI = 0; intI < arItems.Files.length; intI++) {
            if (arItems.Files[intI].Name != "") {
                $('#uploadFileList').append(
                    '<li style="list-style-type: none;background-color: #0a7baf;color: white;padding: 5px;border-bottom: #89b7c1;border-bottom-style: solid;border-bottom-width: 1px;">' +
                    '<span>' + arItems.Files[intI].Name + '</span>' +
                    ' <small style="opacity:.75;">(' + (arItems.Files[intI].DocumentType || '') + ')</small>' +
                    ' <a href="#!" class="pull-right" onclick="deleteItem(this)"><i class="fa fa-times text-yellow"></i></a>' +
                    '</li>'
                );
            }
        }
    ...
    if (jsonObj != undefined && jsonObj != null) {
        ...
        if (lsExistFileName != undefined && lsExistFileName != null && lsExistFileName.length > 0) {
            ...
            if (idx > -1) {
                ...
                jsonObj.Files[idx].DocumentType = "";
...
function receivedText() {
    ...
    /* ── capture selected document type ── */
    var selectedDocType = $('#ddlDocumentType').val();
        ...
        if (lsExistFileName == undefined || lsExistFileName == null || lsExistFileName.length == 0) {
            ...
            for (var intF = 0; intF < jsonObj.Files.length; intF++) {
                ...
                if (jsonObj.Files[intF].Name == "") {
                    ...
                    jsonObj.Files[intF].DocumentType = selectedDocType;   /* NEW */
                    $('#datFile').val(JSON.stringify(jsonObj));

                    $('#uploadFileList').append(
                        '<li style="list-style-type: none;background-color: #0a7baf;color: white;padding: 5px;border-bottom: #89b7c1;border-bottom-style: solid;border-bottom-width: 1px;">' +
                        '<span>' + file.name + '</span>' +
                        ' <small style="opacity:.75;">(' + selectedDocType + ')</small>' +
                        ' <a href="#!" class="pull-right" onclick="deleteItem(this)"><i class="fa fa-times text-yellow"></i></a>' +
                        '</li>'
                    );
