...
$('#btnSubmit').click(function () {
    ...
    $('#Division').val($('#divisionid').val());
    $('#Branch').val($('#branchid').val());
    var validator = $("#frmQuotationReport").validate();
    var isValid = validator.form();
    var data = $("#frmQuotationReport").serialize();

    if (isValid) {
        debugger;

        $.post('GetQuotationReportChart', data).done(function (resultData) {
            if (resultData !== null && resultData !== undefined) {
                ...
                $('#divChartContainer').append('<canvas id="barChartQuotation" style="height:250px"></canvas>');
                var barChartCanvas = $('#barChartQuotation').get(0).getContext('2d')
