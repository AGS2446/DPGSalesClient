...
$('#btnSubmit').click(function () {
    ...
    $('#Division').val($('#divisionid').val());
    $('#Branch').val($('#branchid').val());
    $('#WonLostStatus').val($('#winlostStatus').val());
    var validator = $("#frmOrderReport").validate();
    var isValid = validator.form();
    var data = $("#frmOrderReport").serialize();

    if (isValid) {
        debugger;

        $.post('GetOrderReportChart', data).done(function (resultData) {
            if (resultData !== null && resultData !== undefined) {
                ...
                $('#divChartContainer').append('<canvas id="barChartOrder" style="height:250px"></canvas>');
                var barChartCanvas = $('#barChartOrder').get(0).getContext('2d')
